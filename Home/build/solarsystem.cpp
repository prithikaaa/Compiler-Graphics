//write c++ code using graphics library to implement where planets revolve around the sun and also around their axes. use bresenham circle algorithm to create the sun and planets. assign appropiate different colors to sun and 3 planets. use bresenham ellipse algorithm to create orbits of planet around the sun. assign different colors to the orbits. apply animation where planets revolve around the sun and also around their axes. maintain timing as closest to sun wil revolve quickly and so on.
#include <graphics.h>
#include <math.h>
#include <conio.h>
using namespace std;
...
int main() {
    ...
    float angles[] = {0, 0, 0};
    float speed[] = {0.5, 0.3, 0.1}; 

    while (!kbhit()) {
        cleardevice();
        drawPlanet(sunX, sunY, sunR, WHITE);
        for (int i = 0; i < 3; i++) {
